...
from httpx_oauth.clients.github_enterprise import GitHubEnterpriseOAuth2
from httpx_oauth.exceptions import GetIdEmailError
from httpx_oauth.oauth2 import OAuth2Token, RefreshTokenError

HOST = "github.example.com"
client = GitHubEnterpriseOAuth2(HOST, "CLIENT_ID", "CLIENT_SECRET")

PROFILE_ENDPOINT = f"https://{HOST}/api/v3/user"
EMAILS_ENDPOINT = f"https://{HOST}/api/v3/user/emails"


def test_github_enterprise_oauth2():
    assert (
        client.authorize_endpoint == f"https://{HOST}/login/oauth/authorize"
    )
    assert (
        client.access_token_endpoint == f"https://{HOST}/login/oauth/access_token"
    )
    assert (
        client.refresh_token_endpoint == f"https://{HOST}/login/oauth/access_token"
    )
    assert client.revoke_token_endpoint is None
    assert client.base_scopes == ["user", "user:email"]
    assert client.name == "github-enterprise"
    assert client.api_base_url == f"https://{HOST}/api/v3"


def test_github_enterprise_oauth2_custom_host():
    custom_client = GitHubEnterpriseOAuth2("ghe.mycompany.internal", "ID", "SECRET")
    assert (
        custom_client.authorize_endpoint
        == "https://ghe.mycompany.internal/login/oauth/authorize"
    )
    assert (
        custom_client.access_token_endpoint
        == "https://ghe.mycompany.internal/login/oauth/access_token"
    )
    assert custom_client.api_base_url == "https://ghe.mycompany.internal/api/v3"


def test_github_enterprise_oauth2_custom_name():
    named_client = GitHubEnterpriseOAuth2(HOST, "ID", "SECRET", name="my-ghe")
    assert named_client.name == "my-ghe"
...
@pytest.mark.asyncio
class TestGitHubEnterpriseRefreshToken:
    @respx.mock
...
class TestGitHubEnterpriseGetIdEmail:
    @pytest.mark.asyncio
    @respx.mock
